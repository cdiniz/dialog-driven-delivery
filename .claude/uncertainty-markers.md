...
- Must have corresponding entry in "Section 7: Open Questions"
...
- Must have corresponding entry in "Section 10: Appendix > Assumptions"
...
1. **User doesn't answer your clarifying questions**
   - You asked in Step 3 but user skipped the question
...
Add to **Section 7: Open Questions**
...
- [ ] **Q1:** Which authentication method should we use?
  ...
  - **Context:** Referenced in FR3 (authentication requirement)
  - **Options:** SSO, password-based, social login
```

### For `[ASSUMPTION]`:

Add to **Section 10: Appendix > Assumptions**
...
Add to **Section 7: Open Questions** (technical specs)
...
- [ ] **All markers have tracking:** Every inline marker has corresponding entry in Section 7 or Section 10
...
- For unanswered questions → Use `[OPEN QUESTION]` or `[CLARIFICATION NEEDED]`
- For reasonable inferences → Use `[ASSUMPTION]`
- For vague requirements → Use `[CLARIFICATION NEEDED]`

**Step 6.5: Uncertainty Validation** (NEW)
- Count all uncertainty markers
- Verify each has tracking entry
- Ask user: "Resolve now or proceed with [X] uncertainties?"

---

### In `/create-technical-spec`:

**Step 6: Analyze Technical Input**
- Extract decisions from transcript
- Identify unresolved technical choices

**Step 8: Generate Technical Specification**
- For unresolved decisions → Use `[DECISION PENDING]`
- For inferred approaches → Use `[ASSUMPTION]`
- For unclear constraints → Use `[CLARIFICATION NEEDED]`

**Step 9: Uncertainty Validation** (NEW)
- Validate all markers tracked in Open Questions
- Warn if critical decisions are pending

---

### In `/decompose-feature`:

**Step 8: Propose Story Breakdown**
- If story boundaries unclear → Use `[CLARIFICATION NEEDED]` in story description
- If acceptance criteria vague → Mark with `[OPEN QUESTION]`

**Step 9: Create User Stories**
- Stories with uncertainty markers should be flagged for refinement before sprint planning
...
|--------|----------|------------|-----------|
| `[OPEN QUESTION]` | User decision needed | Section 7 | Yes - resolve before implementation |
| `[DECISION PENDING]` | Technical choice deferred | Section 7 | Yes - resolve during technical planning |
| `[ASSUMPTION]` | Inference made, needs validation | Section 10 | No - but validate before implementation |
| `[CLARIFICATION NEEDED]` | Vague requirement | Section 7 | Yes - clarify before implementation |
