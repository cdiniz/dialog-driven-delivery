...
This command creates a **single specification document** containing BOTH Product and Technical specifications. Fill only what you know from the context - empty sections are better than hallucinated content. The specs will grow progressively as more information becomes available through `/refine-spec`.
...
5. Create specification with both spec sections
...
**Store provider name** for use in Steps 2 and 7.
...
### Step 2: Get Specification Location

After receiving the input, ask for specification location:

```markdown
Where would you like to create this specification?

**Location:** [Ask for location key or name, e.g., "PROJ" or "Engineering"]
**Parent Page (optional):** [Ask if they want this under a specific parent page]

If you're not sure about the available locations, I can list them for you.
```

**If user needs list:** Use Skill tool to invoke spec provider:
```
Skill(skill="[provider-name]", args="list_locations")
```

The provider will return available locations.
...
Should I create this specification in [LOCATION-KEY]?
```
...
Before generating the spec, invoke the `uncertainty-markers` skill if you need guidance on marking unknowns and assumptions.
...
Create the specification with the complete content:

**Use Skill Tool to invoke spec provider:**
```
Skill(
  skill="[provider-name]",
  args="create_spec location_id=\"[LOCATION-KEY]\" title=\"[Page Title]\" body=\"[Full Spec Content]\" parent_id=\"[Optional Parent ID]\""
)
```

**Parameters:**
- **location_id:** Location key/ID from Step 2
- **title:** Confirmed page title
- **body:** Full combined specification (both Product and Technical sections) in Markdown format
- **parent_id:** Optional parent page ID if specified

**The provider will:**
- Handle any platform-specific preprocessing (e.g., Confluence empty bracket removal)
- Convert Markdown to native format if needed
- Create the specification page
- Return: `{id, url, title, version}`
...
1. Review the specification: [URL]
2. [If gaps exist] Schedule sessions to discuss:
   - [Missing product aspects]
   - [Missing technical aspects]
3. Use `/refine-spec` to add information as it becomes available
4. Once specs are complete, use `/decompose` to create user stories
...
4. **Specification creation fails**: Provide full specification text for manual creation
5. **Location not found**: List available locations and ask user to select one
6. **Provider invocation fails**: Fall back to providing spec content for manual creation
...
- **Always creates both specs**: Every specification gets Product AND Technical sections
...
- **Single source of truth**: Everything in one specification document
- **Natural workflow**: Accepts any type of input without forcing structure
- **Provider-agnostic**: Uses Markdown format, works with any specification storage platform
