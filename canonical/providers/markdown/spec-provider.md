...
This skill reads configuration from {{config_file}} in the current working directory.

**Expected format in {{config_file}}:**
```markdown
### Spec Provider
...
**Important:** All operations read this configuration at runtime. If {{config_file}} is not found or doesn't contain the configuration section, defaults are used
...
**Implementation:**
Use {{glob_tool}} to find directories in {specs_dir}/, or return root location.
...
4. Write body to file using {{write_tool}} tool
5. Return metadata
...
**Implementation:**
1. Find file using {{read_tool}} tool (always scope to specs directory):
   - If page_id starts with `{specs_dir}/`: try exact path
   - Try `{specs_dir}/{page_id}`
   - Try `{specs_dir}/{page_id}.md`
   - Use {{glob_tool}} pattern `{specs_dir}/**/*{page_id}*.md` as fallback
2. Read file content
...
2. Write updated body using {{write_tool}} tool
...
**Implementation:**
1. Use {{search_tool}} to search markdown files in {specs_dir}/ (or specified location)
...
- Use {{read_tool}}/{{write_tool}} tools for file operations
- Use {{glob_tool}} for finding files
- Use {{search_tool}} for searching content
- Git operations via {{bash_tool}} (optional, only if version_message provided)
