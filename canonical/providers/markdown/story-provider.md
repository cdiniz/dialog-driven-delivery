...
2. Use {{glob_tool}} to find existing stories in that directory to determine next ID
...
4. **Transform generic template to markdown format:**
   ...
   **Key responsibilities:**
   ...
   - **Body structure:** From template (configurable via {{config_file}})
...
**Implementation:**
1. Use {{glob_tool}} to find story files by ID
...
- Use {{glob_tool}} to find stories: `stories/**/story-*.md`
- Use {{search_tool}} to search by status, labels, dependencies
...
- **{{glob_tool}}:** Find story files
- **{{search_tool}}:** Search by status, labels, dependencies
- **{{bash_tool}}:** Git operations
