...
The Brain Source setting points D3 at a team knowledge repo (an "llm-wiki" or "team brain") — a separate location where raw inputs like meeting transcripts and slack threads are ingested and distilled into a navigable wiki. When set, `d3:create` and `d3:refine` can read context from it instead of requiring pasted transcripts.
